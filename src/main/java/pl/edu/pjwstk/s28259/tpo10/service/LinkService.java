...
public class LinkService {
    ...
    public Link create(LinkRequest linkRequest){
        return create(linkRequest.getPassword(), linkRequest.getName(), linkRequest.getTargetUrl());
    }
    /**
     * Use this method to create a new Link object with an autogenerated unique id
     */
    public Link create(String password, String name, String targetUrl){
        Optional<Link> existingLink = linkRepository.findByTargetUrl(targetUrl);
        if(existingLink.isPresent()){
            LinkResponse linkResponse = toResponseDto(existingLink.get());
            throw new DuplicateTargetUrlException(linkResponse);
        }

        List<String> ids = linkRepository.findAllIds();
        String shortUrlId = generateRandomId();

        while( ids.contains(shortUrlId) )
    ...
    public Link save(LinkRequest newLinkRequest) {
        return linkRepository.save(create(newLinkRequest));
    }
    ...
    public void updateLink(String id, @Valid LinkRequest linkRequest) {
        Optional<Link> optionalLink = linkRepository.findLinkById(id);
        if(optionalLink.isPresent()){
            Link link = optionalLink.get();
            updateLink(link, linkRequest);
        }
    }
    
    public void updateLink(Link link, @Valid LinkRequest linkRequest) {
